...
Gerado em 2026-08-12T20:52:15.722Z · commit `f91e0519` · referência visual: **copa2026** (golden master, ver CLAUDE.md).
...
| Linha de ranking (.rank-row) | gridTemplateColumns | A regra CSS `grid-template-columns: 48px 1fr auto auto` e IDENTICA nos tres apps. A 4a trilha `auto` resolve para a largura real do botao 'Ver palpites' quando ele existe, e colapsa para 0px quando nao existe -- e ele so aparece depois do prazo de palpites. Medido em 2026-08-12 (tarde): copa2026 e cdb2026 renderizam o botao (99.3125px) e br2026 nao (0px). Largura de trilha dirigida por CONTEUDO, nao por token de design. HISTORICO: esta entrada foi REMOVIDA na manha de 2026-08-12 porque a medicao daquele momento dava EQUAL nos tres -- o cdb2026 ainda nao tinha prazo publicado nem chaveamento derivado, entao tambem colapsava. Com as quartas abertas e a semifinal prevista, ele passou a renderizar como a Copa e a divergencia voltou. A remocao estava certa para o estado de entao; o que faltou foi notar que a condicao era de ESTADO do torneio, e voltaria. [docRef: ALLOWLIST.json ranking-row:height entry above (same root cause); owner: Platform; approvedBy: Eduardo; reviewDate: 2026-08-12; reviewBy: 2027-08-03] |
...
| backgroundColor | `rgba(0, 0, 0, 0)` | `rgba(0, 0, 0, 0)` | `rgba(0, 0, 0, 0)` | EQUAL | — |
| color | `rgb(238, 247, 241)` | `rgb(238, 247, 241)` | `rgb(238, 247, 241)` | EQUAL | — |
| height | `8328.97px` | `1797px` | `4653.88px` | JUSTIFIED | Total rendered page length — a function of how much content each app currently has loaded (fixture size, number of phases/rounds/results), not a fixed design token. Comparing it as if it were a token would flag a DIVERGENT finding on every future content change in any app, forever, with no CSS fix possible. PR120-final review item 3 explicitly instructs: 'não compare altura total de main'. [docRef: PR120-final review item 3 (verbatim task text); docs/bolao/PLATFORM_GOVERNANCE.md; owner: Platform; approvedBy: Eduardo; reviewDate: 2026-08-03; reviewBy: 2027-08-03] |
| minHeight | `0px` | `0px` | `0px` | EQUAL | — |
| gridTemplateColumns | `none` | `none` | `none` | EQUAL | — |
...
| minHeight | `0px` | `0px` | `0px` | EQUAL | — |
| gridTemplateColumns | `48px 882.484px 18.2031px 99.3125px` | `48px 981.797px 18.2031px 0px` | `48px 882.484px 18.2031px 99.3125px` | JUSTIFIED | A regra CSS `grid-template-columns: 48px 1fr auto auto` e IDENTICA nos tres apps. A 4a trilha `auto` resolve para a largura real do botao 'Ver palpites' quando ele existe, e colapsa para 0px quando nao existe -- e ele so aparece depois do prazo de palpites. Medido em 2026-08-12 (tarde): copa2026 e cdb2026 renderizam o botao (99.3125px) e br2026 nao (0px). Largura de trilha dirigida por CONTEUDO, nao por token de design. HISTORICO: esta entrada foi REMOVIDA na manha de 2026-08-12 porque a medicao daquele momento dava EQUAL nos tres -- o cdb2026 ainda nao tinha prazo publicado nem chaveamento derivado, entao tambem colapsava. Com as quartas abertas e a semifinal prevista, ele passou a renderizar como a Copa e a divergencia voltou. A remocao estava certa para o estado de entao; o que faltou foi notar que a condicao era de ESTADO do torneio, e voltaria. [docRef: ALLOWLIST.json ranking-row:height entry above (same root cause); owner: Platform; approvedBy: Eduardo; reviewDate: 2026-08-12; reviewBy: 2027-08-03] |
