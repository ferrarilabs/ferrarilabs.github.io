...
| 5 | Every shipped button wired to a real RPC or documented real API call | **testado e executado** (via code read + `node -c` syntax check) | All 11 sections built. Participantes, Pagamentos, Sorteios, Bilhetes, Resultados, Publicações, E-mails wired to real RPCs. Visão geral is read-only (pure aggregate SELECTs, no buttons). Auditoria and Saúde do sistema both call the real `verify_powerball_audit_chain` RPC. Comprovantes calls the real Supabase Storage API (`list()/upload()/createSignedUrl()`) against the not-yet-created `powerball-private` bucket — every action currently fails with a real "Bucket not found" error rather than a fake success, by design (see POWERBALL_ADMIN_STORAGE.md). `node -c admin/js/app.js` confirms valid syntax; grep confirms zero `.insert()/.update()/.delete()` calls anywhere in the file (Storage calls are `.storage.from(...)`, a different API surface, not a table write). |
...
| 9 | RLS deny-by-default, real negative tests written | **proposto, mas não executado** | `tests/rls_negative_test.mjs` — real runnable code, 9 sub-assertions: anon write/read/RPC blocked (participant table + participant RPC), roleless blocked, auditor read-only + blocked from `admin_create_participant`, admin RPC-only (raw INSERT rejected even though authorized), audit table immutable even for owner, anon blocked from 6 of the newer RPCs (`admin_record_payment`, `admin_create_draw`, `admin_create_ticket`, `admin_record_result`, `admin_publish_tickets`, `admin_enqueue_email`), auditor blocked from `admin_record_payment` and from the critical `admin_publish_tickets`. Ran just now → `SKIPPED (NÃO EXECUTADO)`, exit code 2. Output: `tests/rls_negative_test.output.txt`. |
