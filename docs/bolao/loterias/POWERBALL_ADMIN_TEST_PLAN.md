...
| 5 | Every shipped button wired to a real RPC | **testado e executado** (via code read + `node -c` syntax check) | Participantes, Pagamentos, Sorteios, Bilhetes, Resultados (all as before), plus Publicações ("Publicar bilhetes" → `admin_publish_tickets`, requires typing CONFIRMAR) and E-mails ("Enfileirar e-mail" → `admin_enqueue_email`, "Tentar novamente" → `admin_retry_email`, "Cancelar" → `admin_cancel_email_job`) are the seven wired screens. All other 4 sections render text only, no `<button>` elements. `node -c admin/js/app.js` confirms valid syntax; grep confirms zero `.insert()/.update()/.delete()` calls anywhere in the file. |
