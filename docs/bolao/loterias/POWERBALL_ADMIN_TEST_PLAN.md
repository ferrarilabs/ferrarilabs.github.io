...
| 5 | Every shipped button wired to a real RPC | **testado e executado** (via code read) | Only Participantes screen has buttons; "Novo participante" → `admin_create_participant`, "Arquivar" → `admin_archive_participant`. All other 10 sections render text only, no `<button>` elements — confirmed by reading `admin/js/app.js`'s `renderNotImplemented()`. |
