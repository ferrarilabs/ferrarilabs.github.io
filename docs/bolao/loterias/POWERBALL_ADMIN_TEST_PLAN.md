...
| 5 | Every shipped button wired to a real RPC | **testado e executado** (via code read + `node -c` syntax check) | 10 of 11 sections built: Participantes, Pagamentos, Sorteios, Bilhetes, Resultados, Publicações, E-mails (all as before), plus Visão geral (read-only, no buttons — pure aggregate SELECTs), Auditoria (read-only list + "Verificar cadeia de auditoria" → real `verify_powerball_audit_chain` RPC call), and Saúde do sistema (read-only checks incl. the same audit-chain RPC). Only Comprovantes remains text-only/unbuilt (depends on a Storage bucket that doesn't exist yet). `node -c admin/js/app.js` confirms valid syntax; grep confirms zero `.insert()/.update()/.delete()` calls anywhere in the file. |
