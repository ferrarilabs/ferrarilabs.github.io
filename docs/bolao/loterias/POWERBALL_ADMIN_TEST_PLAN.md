...
| 5 | Every shipped button wired to a real RPC | **testado e executado** (via code read + `node -c` syntax check) | Participantes ("Novo participante" → `admin_create_participant`, "Arquivar" → `admin_archive_participant`), Pagamentos ("Registrar pagamento" → `admin_record_payment`, "Reverter" → `admin_reverse_payment`), Sorteios ("Novo sorteio" → `admin_create_draw`, "Editar estimativas" → `admin_update_draw_estimates`) are the three wired screens. All other 8 sections render text only, no `<button>` elements — confirmed by reading `admin/js/app.js`'s `renderNotImplemented()`. `node -c admin/js/app.js` confirms valid syntax; grep confirms zero `.insert()/.update()/.delete()` calls anywhere in the file. |
