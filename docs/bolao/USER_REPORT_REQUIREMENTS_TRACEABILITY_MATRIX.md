...
| R-REPORT-016 | Observabilidade sem conteúdo | Eventos agregados; classe de redação, nunca valor | `workers/user-report-intake/src/index.ts` (`metrica`) | `scripts/report/test_worker_intake.mjs` — "nenhum log carrega relato…"; "…registra a CLASSE, nunca o valor". **Aceitação sintética 2026-08-25:** logs reais só `report_metrica:aceito` e `:idempotente`; sem conteúdo, sem chave, 0 exceções | IMPLEMENTADO · VERIFICADO EM PRODUÇÃO |
