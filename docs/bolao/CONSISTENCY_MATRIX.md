...
|---|---|---|---|---|---|
| Seta de movimento do ranking de participantes | Duas implementações concorrentes: `computeRankArrows()`/`_rankArrowState` (sessão, baseline = último render — **imperfeita**) e `liveMatchPointsTable()` (stateless, oficial-vs-provisório — **correta**, mas só usada dentro do detalhe de uma partida, não no ranking geral) | Nova: `calculateRankingMovement()`, stateless, baseline = janela de partidas ao vivo, reusa `rankEntries()` (mesmo comparador do ranking exibido) | Não existe ainda | **INTENTIONALLY_DIFFERENT** (Copa, ratificado no encerramento da Issue #166 em 2026-08-20) / `CONSISTENT` (BR2026, novo, correto) | Copa: a decisão de 2026-07-13 foi deliberada — o BR2026 usaria o padrão correto e a Copa ficaria como está. Reavaliada em 2026-08-20 ao encerrar a Issue #166: `bolao/copa2026/js/app.js:1968` continua com `_rankArrowState`, `git log --all -S "_rankArrowState"` devolve exatamente um commit (o original, `03b67d54`), o app está arquivado (`CONFIG.archived: true`) e `/bolao/` redireciona para `/bolao/br2026/`. A tela de ranking da Copa não liquida dinheiro nem tem usuário ativo, então portar o padrão seria risco sem ganho. Registrado como divergência aceita, não como dívida pendente — era `NEEDS_REVIEW` desde julho e ninguém iria revisá-la. Se a Copa for reativada, portar `calculateRankingMovement()` do BR2026 em vez do `Map` de sessão. CDB2026: adicionar quando fizer sentido (torneio de eliminação simples, sem "rodadas" no mesmo sentido) |
...
| Divergência | Categoria | Justificativa |
|---|---|---|
| `CONFIG.entryRosterFrozen` + `isEntryCreationAllowed()`/`editingEntryIsValid()`/`updateExistingEntry()` existem só no CDB2026 | **TOURNAMENT_SPECIFIC — INTENTIONALLY_DIFFERENT** | Só o CDB2026 precisa desacoplar permanentemente **criação de entrada** do relógio/cutoff de fase. **Regra atual: o CDB2026 NÃO reabre palpites de participante.** Esta flag existe para que um cutoff futuro/configuração de fase não faça `isPastEntryCutoff()` voltar a `false` e, por consequência, reabra acidentalmente o cadastro de novas entradas. Não inferir desta flag nenhuma janela de reabertura de palpites. A Copa2026 está arquivada (`CONFIG.archived`) e o BR2026 encerrou inscrições em 2026-07-16; não propagar sem uma necessidade equivalente de congelar o roster independentemente do cutoff. |
