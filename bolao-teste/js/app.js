...
function renderParticipants() {
  ...
  s.entries.forEach(e => {
    const div = document.createElement("div");
    div.className = "rank-row";
    div.innerHTML = `<div class="medal">👤</div><div><b>${e.entryName}</b><br><span class="muted">${e.payerName}${e.paymentMethod ? " • " + e.paymentMethod : ""}</span></div><div class="status">${state().paid[e.id] ? "Pago" : "Pendente"}</div>`;
    box.appendChild(div);
  });
}
