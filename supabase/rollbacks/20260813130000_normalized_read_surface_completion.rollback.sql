...
-- ROLLBACK for 20260813130000_normalized_read_surface_completion.sql
--
-- Restores the read surface to exactly M16, and removes every privilege this campaign granted
-- to a browser role. Ordered so the surface stops being reachable BEFORE the projection that
-- feeds it changes: drop the public view first, and no anon request can observe an
-- intermediate shape.
--
-- PRECONDITION, and it is the only one that matters: no application may be routed at
-- public.bolao_state_normalized_public when this runs. Rolling the surface back while a live
-- app reads it would blank three products. The read route is a config edit in each app and is
-- reverted FIRST — see the READ_CUTOVER rollback procedure. This file is the second step.
--
-- It writes no row and drops no table, so it is FULL: after it, the database is bit-identical
-- to its state before 20260813130000. The three preceding migrations are NOT undone by this
-- file and must not be: their columns and rows are source-backed facts, and dropping them
-- would discard the 50 payment confirmations and the 8 tombstones to undo a projection.

BEGIN;

DROP VIEW IF EXISTS public.bolao_state_normalized_public;

-- Back to M16: SECURITY INVOKER, owned by postgres, service_role only.
ALTER FUNCTION bolao.read_document(text) SECURITY INVOKER;
ALTER FUNCTION bolao.read_document(text) OWNER TO postgres;

DROP POLICY IF EXISTS pools_public_reader_select ON bolao.pools;
REVOKE ALL ON TABLE bolao.pools FROM bolao_public_reader;
DROP POLICY IF EXISTS pool_entries_public_reader_select ON bolao.pool_entries;
REVOKE ALL ON TABLE bolao.pool_entries FROM bolao_public_reader;
DROP POLICY IF EXISTS pool_entry_tombstone_public_reader_select ON bolao.pool_entry_tombstone;
REVOKE ALL ON TABLE bolao.pool_entry_tombstone FROM bolao_public_reader;
DROP POLICY IF EXISTS predictions_public_reader_select ON bolao.predictions;
REVOKE ALL ON TABLE bolao.predictions FROM bolao_public_reader;
DROP POLICY IF EXISTS classification_predictions_public_reader_select ON bolao.classification_predictions;
REVOKE ALL ON TABLE bolao.classification_predictions FROM bolao_public_reader;
DROP POLICY IF EXISTS matches_public_reader_select ON bolao.matches;
REVOKE ALL ON TABLE bolao.matches FROM bolao_public_reader;
DROP POLICY IF EXISTS match_results_public_reader_select ON bolao.match_results;
REVOKE ALL ON TABLE bolao.match_results FROM bolao_public_reader;
DROP POLICY IF EXISTS ties_public_reader_select ON bolao.ties;
REVOKE ALL ON TABLE bolao.ties FROM bolao_public_reader;
DROP POLICY IF EXISTS competition_edition_phases_public_reader_select ON bolao.competition_edition_phases;
REVOKE ALL ON TABLE bolao.competition_edition_phases FROM bolao_public_reader;
DROP POLICY IF EXISTS sync_state_public_reader_select ON bolao.sync_state;
REVOKE ALL ON TABLE bolao.sync_state FROM bolao_public_reader;
DROP POLICY IF EXISTS entry_payment_confirmation_public_reader_select ON bolao.entry_payment_confirmation;
REVOKE ALL ON TABLE bolao.entry_payment_confirmation FROM bolao_public_reader;

REVOKE ALL ON SCHEMA bolao FROM bolao_public_reader;
DROP ROLE IF EXISTS bolao_public_reader;

-- M16's function body, restored verbatim from the frozen spec rather than retyped.
