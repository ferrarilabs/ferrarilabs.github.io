...
--
-- 20260812120000_expand_m15_match_location_tie_lock_provenance_and_the_official_draw.sql
--
-- EXPAND stage M15 — match location, tie lock provenance and the official draw
--
...
--
-- ROLLBACK (FULL_BEFORE_BACKFILL). FULL. ALTER TABLE ... DROP COLUMN, five times. Safe while the columns are empty; once the backfill has run, dropping them discards the only normalized copy of operator lock provenance and the official draw record — so after backfill this is FORWARD_FIX_ONLY, for the same reason M8 is.
--
-- ============================================================
