 ...
 *
 * Ao editar a lista, lembre do `?v=`: este arquivo está registrado em `SHARED_FILES` de
 * `bolao/scripts/cachebust.mjs`, então o bot re-tagueia sozinho. Não remova esse registro (é o
 * achado F18: módulo compartilhado sem `?v=` fica preso no cache do navegador).
 */
(function (root) {
  "use strict";

  /**
   * Transmissões CONFIRMADAS. `espnId` é a chave forte (BR2026 carrega o id do evento ESPN);
   * `kickoffUtc` + times é o fallback para o CDB2026, cujo objeto de partida em runtime não tem id.
   */
  var BROADCASTS = [
    {
      espnId: "401913077",
