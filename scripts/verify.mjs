...
const CHECKS = [
  ...
  { id: "sentinel-hardening", group: "security", cmd: ["node", "scripts/sentinel/test_sentinel_hardening.mjs"],
    why: "os tres consertos que devolvem sentido ao alarme. #404: Projects v2 indisponivel (credencial que o GITHUB_TOKEN nao pode ter) era contado como erro de reconciliacao e deixou o Sentinel VERMELHO em todo run desde 2026-08-29 — status permanentemente vermelho nao e alarme, e ruido, e foi assim que ele atravessou o #396 sem avisar; o predicado e ESTREITO e qualquer outro erro continua reprovando. #405: heartbeat que so conta event=schedule, porque no #396 o produtor rodava a cada 5 min por dispatch externo enquanto todo o cron estava morto ha 45 h; limiar de 6 h MEDIDO (o maior intervalo saudavel real foi 3.12 h, entao 3 h daria alarme falso). #406: fase ativa decidida + sucessora com topologia e zero confrontos materializados = proximo resultado nao seria descoberto, com cron saudavel — e NAO alerta por partida sem data, que e o caso legitimo da #395" },
  ...
  { id: "cdb-materialize-derived-phase", group: "app",
    cmd: ["python3", "bolao/cdb2026/scripts/test_materialize_derived_phase.py"],
    why: "materializar fase derivada so pode derivar de topologia CBF + vencedor persistido, e nunca inventar clube, data ou local" },
  { id: "cdb-next-known-undated", group: "app",
    cmd: ["node", "bolao/cdb2026/scripts/test_next_known_undated.mjs"],
    why: "data desconhecida nao pode fazer confronto conhecido sumir — e o fallback nao pode inventar data, local, transmissao nem escudo" },
  { id: "cdb-operator-audit-trail", group: "app",
    cmd: ["python3", "bolao/cdb2026/scripts/test_operator_audit_trail.py"],
    why: "comando de operador que diz 'feito' sem deixar rastro no servidor e pior que um que falha" },
  { id: "cdb-backfill-venue", group: "app",
    cmd: ["python3", "bolao/cdb2026/scripts/test_backfill_venue.py"],
    why: "preencher local nunca pode virar reescrever resultado, e curadoria vence provedor" },
  ...
  { id: "cdb-legacy-derived-pick-continuity", group: "browser", cmd: ["node", "bolao/cdb2026/scripts/test_legacy_derived_pick_continuity.mjs"],
    why: "#428 (2026-09-10, achado contra dado real de producao -- 0 de 12 entradas mostravam a semifinal): as 12 entradas reais palpitaram a semifinal ANTES da materializacao, sob o id de SLOT da topologia (sf-1/sf-2), nunca sob o id real do confronto -- o unico caminho que existiu de verdade neste torneio. Sem legacyDerivedTieIds()/pickByTieId(), renderPickDisplay/scoreEntry/explainScore procuravam so pelo id novo e nao achavam nada: \"Ver palpites\" ficava sem a semifinal e scoreEntry() teria pontuado ZERO nos jogos e no bonus de classificacao quando o resultado saisse, silenciosamente" },
  ...
  { id: "broadcasts-validate", group: "app", cmd: ["node", "bolao/shared/scripts/validate_broadcasts.mjs"],
